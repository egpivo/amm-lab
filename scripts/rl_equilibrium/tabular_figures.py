...
def fig_m2(rows: list[dict]) -> None:
    panels = [
        ...
        (
            ...
            "dynamic-fee coefficient perturbation",
        ),
    ]
    ...
    for ax, (diag, variants, title) in zip(axes.flat, panels):
        ...
        ax.set_xticks(
            range(len(variants)),
            variants,
            rotation=20 if diag == "coeff" else 0,
            fontsize=8,
        )
...
def main() -> None:
    m1 = read_csv(OUT / "m1_results.csv")
    fig_comparison(m1)
    fig_paired(m1)
    fig_trajectory(read_csv(OUT / "m1_trajectory_q.csv"))
    fig_actions(read_csv(OUT / "m1_actions.csv"))
    fig_m2(read_csv(OUT / "m2_diagnostics.csv"))
    print(
        "wrote m1_comparison, m1_paired_diff, m1_trajectory, m1_actions_time, "
        "m1_actions_state, m2_artifacts to",
        OUT,
    )
