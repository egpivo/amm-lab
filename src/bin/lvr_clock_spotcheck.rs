//! Round-24 five-second spot-check: certifies (or rejects) the
//! 15-second clock as the primary market clock.
//!
...
const N5: usize = 120_960; // 5-second steps per week
const CAL_SEEDS: std::ops::Range<u64> = 500..503; // pilot domain
const EVAL_SEEDS: std::ops::Range<u64> = 2000..2020; // pilot domain (fresh)
...
fn gen_base(seed: u64, sigma: f64, lam_fund: f64, lam_arb: f64) -> Base {
    let mut rng = StdRng::seed_from_u64(seed ^ 0x5CA1_E5CA);
    let dt5_years = (1.0 / 720.0) / (24.0 * 365.0);
    let normal = Normal::new(0.0, 1.0).unwrap();
    let mut path = Vec::with_capacity(N5 + 1);
    let mut acc = 0.0;
    path.push(acc);
    for _ in 0..N5 {
        acc +=
            -0.5 * sigma * sigma * dt5_years + sigma * dt5_years.sqrt() * normal.sample(&mut rng);
        path.push(acc);
    }
...
fn bin_base(base: &Base, ck: &Clock) -> (Vec<f64>, InjectedSchedules) {
    let prices: Vec<f64> = (0..=ck.n_steps)
        .map(|j| S0 * base.log_path5[j * ck.agg].exp())
        .collect();
...
fn config_for(fee: f64, sigma: f64, z: f64, ck: &Clock) -> SimConfig {
    ...
    SimConfig {
        name: "spotcheck".into(),
        description: "round-24 5s spot-check".into(),
        amm_fee: fee,
        cex_fee: 0.0010,
        buy_demand: z * d_ref,
        sell_demand: z * d_ref,
...
fn main() {
    ...
    for (sname, fee, target, arb) in cells {
        ...
        // hazard fixed at the 15s-benchmark calibration
        let ck15 = &clocks()[0];
        ...
        for p in 0..2 {
            // static, gap: level criteria
            let (a15, b15, u15, s15, al15) = mean[0][p];
            let (a5, b5, u5, s5, al5) = mean[1][p];
            let checks = [
        ...
        // defensive: absolute fallbacks
        let (a15, _, u15, s15, _) = mean[0][2];
        let (a5, _, u5, s5, _) = mean[1][2];
        let da = (a15 - a5).abs() / V0;
        let du = (u15 - u5).abs() / V0;
        let shut15 = s15 < 0.01 * mean[0][0].3;
        let shut5 = s5 < 0.01 * mean[1][0].3;
        ...
        for i in 0..3 {
            for j in (i + 1)..3 {
                ...
                let d15: f64 = (0..n_eval)
                    ...
                    .sum::<f64>()
                    / n_eval as f64;
                let near_tie = d5.abs() / V0 < 2.0e-5;
                let agree = d5.signum() == d15.signum();
                if !agree && !near_tie {
                    cell_pass = false;
                }
                println!(
                    "    pair {}-{}: d5/V0={:.2e} d15/V0={:.2e} near_tie={near_tie} agree={agree}{}",
                    names[i],
                    names[j],
                    d5 / V0,
                    d15 / V0,
    ...
    println!(
        "\n== SPOT-CHECK VERDICT: {} ==",
        if all_pass {
            "ALL FOUR CELLS PASS — freeze the 15-second primary clock"
        } else {
            "FAILED — do not freeze 15 s; fall back to 5 s or run a finer local diagnostic"
        }
    );
}
